...
app.put('/orders/:id/cancel', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!order) {
      return res.status(404).json({ error: 'Order not found, or it does not belong to you.' });
    }

    // Prevent cancelling if already Delivered or Cancelled
